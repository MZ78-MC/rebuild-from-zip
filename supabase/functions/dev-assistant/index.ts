...
serve(async (req) => {
  ...
  try {
    ...
    // Priority: Lovable AI (auto-configured) → OpenAI (best accuracy) → Anthropic → Gemini → OpenRouter → Groq
    ...
    // Get relevant context from past queries using similarity search
    // For now, we'll get recent relevant queries (in production, use vector similarity)
    ...
    // Check if query is financial-related and route to finance-assistant
    const financialKeywords = [
      "spent", "spend", "expense", "income", "salary", "budget", "money", "rans",
      "r ", "r", "save", "saving", "goal", "financial", "finance", "receipt",
      "bill", "payment", "paid", "cost", "price", "groceries", "rent", "petrol",
      "fuel", "transport", "utilities", "shopping", "dining", "takeout"
    ];
    ...
    if (isFinancialQuery) {
      // Route to finance-assistant via HTTP call
      try {
        const functionUrl = `${supabaseUrl}/functions/v1/finance-assistant`;
        const financeResponse = await fetch(functionUrl, {
          method: "POST",
          headers: {
        ...
        if (!financeResponse.ok) {
          throw new Error(`Finance assistant returned ${financeResponse.status}`);
        }

        const financeData = await financeResponse.json();
        return new Response(
          JSON.stringify({
            response: financeData.response || financeData.error || "Financial query processed",
            routed: true,
            transactionId: financeData.transactionId,
            goalId: financeData.goalId,
          }),
          {
        ...
        console.error("Finance assistant routing error:", financeError);
        // Fall through to regular dev assistant if finance routing fails
      }
    }
...
Note: If the user asks about finances, money, expenses, income, or budgets, acknowledge that you've routed their query to the Budget Buddy module.`;
    ...
    if (!aiResponse.ok) {
      ...
      if (aiResponse.status === 401) {
        throw new Error(
          "LOVABLE_API_KEY is invalid or missing. Please check your Supabase Edge Functions environment variables."
        );
      }
    ...
    try {
      ...
      if (memoryError) {
        console.error("Failed to save to dev_memory:", memoryError);
      } else {
        console.log("Successfully saved to dev_memory:", memoryData?.id);
      }
    } catch (memoryError) {
    ...
    try {
      // Note: Supabase vector type expects array format
